...
class FfilterBarr extends Component {
  ...
  getSources = source => {
    if (!this.state.sources.includes(source)) {
      ...
      this.setState(
        { sources: this.state.sources.replace(source, "") },
        this.letsfetch
      );
    }
  };
    ...
    return (
      <FormGroup row>
        ...
        <FormControlLabel
          control={
            <Switch
              onClick={() => this.getSources(`the-wall-street-journal,`)}
              value="wsj"
            />
          }
          label="WSJ"
...
export default connect(mapStateToProps, mapDispatchToProps)(FfilterBarr);
